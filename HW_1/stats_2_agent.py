    ...
    if len(oa) > 3:
        for i in range(len(oa)):
            ...
            if (len(oa) - i) > 3 and oa[i] == 0 and oa[i + 1] == 2 and oa[i + 2] == 1:
                c021 += 1
        # Саздаем словарь, элементами которого являются доли паттерна в
        # статистической выборке предыдущих ходов оппонента
        d = {'012': c012 / (len(oa) // 3), '021': c021 / (len(oa) // 3)}
        ptrn = '012' if d['012'] > d['021'] else '021'

        # Если доля паттерна выше трети всех ходов, то считаем, что паттерн существует
        # Выявляем какой именно паттерн и вычисляем свой следующий ход
        if d[ptrn] > 0.33:
            if ptrn == '012':
                return (loa + 2) % 3
            else:
